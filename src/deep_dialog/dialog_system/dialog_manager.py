...
class DialogManager:
    ...
    def initialize_episode(self, bot, update):
        ...
        if dialog_config.run_mode < 3:
            update.message.reply_text("New episode, user goal:")
            update.message.reply_text(json.dumps(self.user.goal, indent=2))
        self.print_function(bot = bot, update = update, user_action = self.user_action)
            
        self.agent.initialize_episode()

    def next_turn(self, bot , update, record_training_data=True):
        ...
        self.print_function(bot = bot, update = update,agent_action = self.agent_action['act_slot_response'])
    ...
    def print_function(self, bot = None, update = None, agent_action=None, user_action=None):
        """ Print Function """
            
        if agent_action:
            if dialog_config.run_mode == 0:
                if self.agent.__class__.__name__ != 'AgentCmd':
                    update.message.reply_text ("Turn %d sys: %s" % (agent_action['turn'], agent_action['nl']))
            elif dialog_config.run_mode == 1:
                if self.agent.__class__.__name__ != 'AgentCmd':
                    update.message.reply_text("Turn %d sys: %s, inform_slots: %s, request slots: %s" % (agent_action['turn'], agent_action['diaact'], agent_action['inform_slots'], agent_action['request_slots']))
            elif dialog_config.run_mode == 2: # debug mode
                update.message.reply_text("Turn %d sys: %s, inform_slots: %s, request slots: %s" % (agent_action['turn'], agent_action['diaact'], agent_action['inform_slots'], agent_action['request_slots']))
                update.message.reply_text ("Turn %d sys: %s" % (agent_action['turn'], agent_action['nl']))
            
            if dialog_config.auto_suggest == 1:
                update.message.reply_text('(Suggested Values: %s)' % (self.state_tracker.get_suggest_slots_values(agent_action['request_slots'])))
        elif user_action:
            if dialog_config.run_mode == 0:
                update.message.reply_text ("Turn %d usr: %s" % (user_action['turn'], user_action['nl']))
            elif dialog_config.run_mode == 1:
                update.message.reply_text ("Turn %s usr: %s, inform_slots: %s, request_slots: %s" % (user_action['turn'], user_action['diaact'], user_action['inform_slots'], user_action['request_slots']))
            elif dialog_config.run_mode == 2: # debug mode, show both
                update.message.reply_text ("Turn %d usr: %s, inform_slots: %s, request_slots: %s" % (user_action['turn'], user_action['diaact'], user_action['inform_slots'], user_action['request_slots']))
                update.message.reply_text ("Turn %d usr: %s" % (user_action['turn'], user_action['nl']))
                ...
                if len(user_request_slots) > 0:
                    ...
                    for slot in possible_values.keys():
                        if len(possible_values[slot]) > 0:
                            update.message.reply_text('(Suggested Values: %s: %s)' % (slot, possible_values[slot]))
                        elif len(possible_values[slot]) == 0:
                            update.message.reply_text('(Suggested Values: there is no available %s)' % (slot))
                else:
                    kb_results = self.state_tracker.get_current_kb_results()
                    update.message.reply_text ('(Number of movies in KB satisfying current constraints: %s)' % len(kb_results))
